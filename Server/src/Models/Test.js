...
/** Maps the same Firestore document shape to the Supabase `tests` row shape. */
const toMirrorRow = (id, data) => ({
  id: Number(id) || id,
  name: data.name,
  course: data.course,
  centre: data.centre ?? null,
  centre_key: data.centreKey ?? null,
...
const create = async ({ name, course, centre, testDate, createdBy = null }) => {
  ...
  await mirrorUpsert("tests", toMirrorRow(id, payload));
...
const removeById = async (id) => {
  ...
  await ref.delete();
  await mirrorDelete("tests", "id", Number(id) || id);
  return 1;
};
