...
/** Maps the same Firestore document shape to the Supabase `students` row shape. */
...
const create = async (data) => {
  ...
  await mirrorUpsert("students", toMirrorRow(id, payload));
...
const update = async (id, data) => {
  ...
  const doc = await ref.get();
  await mirrorUpsert("students", toMirrorRow(doc.id, doc.data()));
  return toApiStudent(doc.id, doc.data());
};
...
const destroy = async (id) => {
  ...
  if (!ref) return 0;
  await ref.delete();
  await mirrorDelete("students", "id", Number(id) || id);
  return 1;
};
