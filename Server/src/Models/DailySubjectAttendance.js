...
/** Maps to the Supabase `daily_subject_attendances` row shape (natural key: student_id+subject+date+time). */
const toMirrorRow = (data) => ({
  student_id: Number(data.studentId) || data.studentId,
  name: data.name ?? null,
  centre: data.centre ?? null,
  course: data.course ?? null,
  subject: data.subject,
...
const upsert = async ({
  ...
  await mirrorUpsert("daily_subject_attendances", toMirrorRow(payload), "student_id,subject,date,time");
