...
/** Maps the same Firestore document shape to the Supabase `users` row shape. */
const toMirrorRow = (id, data) => ({
  ...
  role: data.role,
  centre: data.centre ?? null,
  is_vishist: isMitraRole(data.role)
    ? Boolean(normalizeIsVishist(data.role, data.isVishist))
    : null,
  available_days: normalizeAvailableDays(data.availableDays),
  fcm_tokens: Array.isArray(data.fcmTokens) ? data.fcmTokens : [],
  otp_hash: data.otpHash || null,
  otp_expires_at: data.otpExpiresAt ? toDate(data.otpExpiresAt)?.toISOString() ?? null : null,
  otp_attempts: data.otpAttempts || 0,
...
const create = async (data) => {
  ...
  await mirrorUpsert("users", toMirrorRow(id, payload));
...
const update = async (id, data) => {
  ...
  const doc = await ref.get();
  await mirrorUpsert("users", toMirrorRow(doc.id, doc.data()));
  return toApiUser(doc.id, doc.data());
};
...
const destroy = async (id) => {
  ...
  if (!ref) return 0;
  await ref.delete();
  await mirrorDelete("users", "id", Number(id) || id);
  return 1;
};
