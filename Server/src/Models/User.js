...
/**
 * Adds a Web Push subscription object ({ endpoint, keys: { p256dh, auth } }).
 * De-dupes by endpoint (a device re-subscribing gets a new keys pair, so a
 * plain append could otherwise accumulate stale duplicates for the same
 * device).
 */
const addPushSubscription = async (id, subscription) => {
  ...
  const current = Array.isArray(existing.push_subscriptions) ? existing.push_subscriptions : [];
  ...
  const { data: row, error } = await getSupabase()
    ...
    .select("*")
    .single();
  assertNoError(error, "Failed to save device subscription");
  return toApiUser(row);
};

const removePushSubscription = async (id, endpoint) => {
  ...
  const { error } = await getSupabase().from(TABLE).update({ push_subscriptions: next }).eq("id", id);
  assertNoError(error, "Failed to remove device subscription");
};
