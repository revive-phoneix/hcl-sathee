...
--
-- This is the real, primary database. Run once against a fresh project's SQL
-- editor (or via `psql`) before pointing the server at SUPABASE_URL /
-- SUPABASE_SERVICE_ROLE_KEY. The Node model layer under Server/src/Models
-- maps every column back to the exact same camelCase JSON shape the Client
-- expects, so no frontend changes are required.
--
...
create table if not exists users (
  ...
  push_subscriptions jsonb not null default '[]'::jsonb, -- Web Push subscription objects, not FCM tokens
  otp_hash           text,
...
-- ---------------------------------------------------------------- student_id_counters
-- Backs the per-Kendra sequential student ID scheme (RJ-JU-001, RJ-JU-002,
-- ...). One row per Kendra ID; incremented atomically via the RPC function
-- below so concurrent "Add Student" submissions never collide.
create table if not exists student_id_counters (
  kendra_id  text primary key,
  value      integer not null default 0
);

create or replace function increment_student_counter(p_kendra_id text)
returns integer as $$
declare
  new_value integer;
begin
  insert into student_id_counters (kendra_id, value)
  values (p_kendra_id, 1)
  on conflict (kendra_id) do update set value = student_id_counters.value + 1
...
-- project's default privileges the way tables created through the Table
-- Editor UI do — without this, every PostgREST request (including from the
-- service_role key) fails with "permission denied for table X" even though
-- the table exists. Safe to re-run.
...
-- Notes
-- ============================================================================
-- * Firebase/Firestore has been fully retired — this is the real, primary
--   database, read and written directly by the Node server.
-- * `vishist_attendances.firestore_id` is a harmless leftover column from the
--   Firebase migration window (it let the old dual-write mirror match rows by
--   Firestore's random doc id). It's unused now but left in place rather than
--   forcing a destructive column drop on a live table.
